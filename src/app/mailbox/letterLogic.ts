...
async function buildLetter(
  ...
  return { number, quoteText, translation, bookTitle, bookAuthor, svg };
}
...
export async function obtainLetter(): Promise<LetterResult> {
  ...
  if (state && state.lastReceiveDate === today) {
    ...
    return buildLetter(state.receivedCount, text, state.translation, bookTitle, bookAuthor);
  }
  ...
  let translation = '';
  try {
    translation = await translateToEnglish(quote.text);
  } catch (e) {
    console.warn('[Mailbox] 翻译失败，将只展示中文:', e);
    translation = '';
  }
